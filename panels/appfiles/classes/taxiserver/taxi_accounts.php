...
class taxi_accounts
{
	...
	{
		lib( 'bcrypt' );
		$r = DB::getRecord( "SELECT acc_id, password_hash, old_password_hash
			FROM ".self::T."
		...
		if( $r['old_password_hash'] )
		{
			if( !PasswordHash::test( $password, $r['old_password_hash'] ) ){
				return null;
			}

			/*
			 * Generate new hash and clear the old one.
			 */
			DB::updateRecord( self::T,
				array( 'password_hash' => bcrypt( $password ),
					'old_password_hash' => null ),
				array( 'acc_id' => $r['acc_id'] )
			);
