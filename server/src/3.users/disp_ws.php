...
define('T_DISP_WS', 'dispatcher-ws');
...
add_auth_func(function($cid, $str) {
	...
	if( !$m || $m->command != 'auth-dispatcher' ) {
		return null;
	}

	write_message($cid, new message('auth-ok'));
	return new conn_user(T_DISP_WS, 0);
});

add_cmdfunc(T_DISP_WS, 'auth-dispatcher', function() {
	// nothing
});
add_cmdfunc(T_DISP_WS, 'cmd', 'disp_ws::dispatch');
add_cmdfunc(T_DISP_WS, 'ping', function() {
	// nothing
});
...
function disp_broadcast($cmd, $data = [], $disp_id = 0)
{
	...
	if($n == 0) {
		trigger_error("Can't broadcast: no DISP_WS client");
		return false;
	}

	if($n > 1) {
		trigger_error("More than one DISP_WS client");
		return false;
	}
	
	$bmsg = new message('cmd', [
		'cmd' => $cmd,
		'data' => $data,
		'dispatcher_id' => $disp_id
	]);

	write_message($conns[0], $bmsg);
}
